...
class FrameProcessor:
    ...
    def process_loop(self, stop_event: threading.Event) -> None:
        """백그라운드 스레드에서 무한히 작동하는 프레임 분석 루프"""
        import threading
        logger.info("🎬 [FrameProcessor] 비전 분석 루프 백그라운드 구동 개시")
        
        while not stop_event.is_set():
            ret, frame = self._video_source.read()
            if not ret or frame is None:
                # 파일이 끝나면 대기 후 계속 시도(루프 옵션 연동 보장)
                time.sleep(0.03)
                continue
                
            try:
                # 프레임 처리 (설정에 맞춰 cv2 GUI 디스플레이 노출 여부 결정)
                _, events = self.process(frame, show_display=self._show_display)
            except Exception as e:
                logger.error(f"⚠️ [FrameProcessor] 루프 내 프레임 처리 실패: {e}")
                time.sleep(0.03)
