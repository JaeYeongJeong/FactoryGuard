...
export async function analyzeImage(file: File, eventId?: string) {
  const form = new FormData();
  form.append("image", file);
  const query = eventId ? `?event_id=${encodeURIComponent(eventId)}` : "";
  return request<Record<string, unknown>>(`/reports/analyze-with-legal-basis${query}`, { method: "POST", body: form });
}
