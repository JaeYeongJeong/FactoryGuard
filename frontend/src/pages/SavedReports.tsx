...
function SavedReports({ reports, events }: { reports: IncidentReport[]; events: DetectionEvent[] }) {
  ...
  return <div><div className="page-heading report-head"><div><h1>저장된 리포트</h1><p className="page-desc">Vision LLM이 생성한 기본 분석과 법령 근거 리포트를 확인합니다.</p></div><button className="button secondary" onClick={() => window.print()} disabled={!selected}><Printer size={17}/> 인쇄</button></div>
